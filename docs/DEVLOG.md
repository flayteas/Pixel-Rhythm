...
**任务**: 解决清除浏览器缓存后匿名账号丢失问题

**问题**: 匿名 token 仅存储在 localStorage，浏览器清缓存或换设备后永久丢失云存档

**方案**: 恢复码系统

**后端改动** (`functions/api/[[path]].js`):
- `generateRecoveryCode()`：生成 8 位大写字母+数字（排除易混淆字符 0/O/1/I/L）
- 修改 `POST /api/auth/register`：注册时生成恢复码，SHA-256 哈希后存入 `recovery_hash`
- 新增 `POST /api/auth/recover`：验证恢复码哈希，签发新 token，恢复码不变
- 新增 `POST /api/auth/reset-recovery`：已登录用户重新生成恢复码（旧码失效）

**前端改动** (`js/cloud.js`):
- `signInAnonymously()` 注册后弹出全屏恢复码展示页（黄色虚线框 + 等宽大字 + 复制按钮）
- `_showRecoverInput()` 恢复码输入界面（8 位输入框，支持 Enter 提交）
- `recoverAccount(code)` 调用恢复 API，成功后自动拉取云端数据
- 未登录对话框新增「恢复账号」按钮
- 已登录对话框新增「重置恢复码」按钮

**数据库改动** (`worker/schema.sql`):
- `users` 表新增 `recovery_hash TEXT` 列
- 现有数据库需执行 `ALTER TABLE users ADD COLUMN recovery_hash TEXT`

**API 端点汇总**（v62 后）:

| 端点 | 方法 | 认证 | 说明 |
|------|------|------|------|
| `/api/auth/register` | POST | 无 | 注册匿名用户，返回 token + recoveryCode |
| `/api/auth/verify` | GET | Bearer | 验证 token 有效性 |
| `/api/auth/recover` | POST | 无 | 用恢复码找回账号，签发新 token |
| `/api/auth/reset-recovery` | POST | Bearer | 重新生成恢复码 |
| `/api/sync` | GET | Bearer | 拉取云端数据 |
| `/api/sync` | PUT | Bearer | 推送本地数据 |
| `/api/leaderboard/{songKey}` | GET | 无 | 获取排行榜 Top 100 |

**恢复码特性:**
- 8 位字符，字符集 `ABCDEFGHJKMNPQRSTUVWXYZ23456789`（30 种字符）
- 固定不变，恢复后不重新生成（可反复使用同一码）
- 手动重置后旧码立即失效
- 服务端仅存储 SHA-256 哈希，不存明文

**修改文件:**
- `functions/api/[[path]].js` — 新增恢复码生成 + recover/reset-recovery 端点
- `js/cloud.js` — 注册后展示恢复码 + 恢复账号/重置恢复码 UI
- `worker/schema.sql` — users 表新增 recovery_hash 列
- `index.html` — V62 更新日志
- `docs/DEVLOG.md` — 更新
